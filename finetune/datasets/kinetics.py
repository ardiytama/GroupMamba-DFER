...
class VideoClsDatasetFrame(Dataset):
    ...
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
        ...
        # add for 3d face restruction feature
        ...
        import pandas as pd
        cleaned = pd.read_csv(self.anno_path, delimiter=',', dtype={0:str})
        self.dataset_samples = list(cleaned.values[:, 0])
        ...
        if task != 'classification': # regression
            ...
            mafw_dict = {
                'anger': 0, 'anxiety': 1, 'contempt': 2, 'disappointment': 3,
                'disgust': 4, 'fear': 5, 'happiness': 6, 'helplessness': 7,
                'neutral': 8, 'sadness': 9, 'surprise': 10
            }
            if len(raw_labels) > 0 and isinstance(raw_labels[0], str) and not str(raw_labels[0]).isdigit():
                self.label_array = [mafw_dict.get(str(l).lower().strip(), l) for l in raw_labels]
            else:
                if getattr(self.args, 'data_set', None) == 'DFEW':
                    self.label_array = [int(l) - 1 for l in raw_labels]
                else:
                    self.label_array = [int(l) for l in raw_labels]
    ...
    def load_video(self, sample, sample_rate_scale=1):
        ...
        if self.args and getattr(self.args, 'data_set', None) == 'DFEW':
            sample = sample.zfill(5)
        sample = os.path.join(self.data_path, sample)
        fname = sample


        if len(os.listdir(fname))<1:
            return []
...
class VideoClsDatasetSegment(VideoClsDatasetFrame):
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
                 frame_sample_rate=2, crop_size=224, short_side_size=256,
                 new_height=256, new_width=340, keep_aspect_ratio=True,
                 num_segment=1, num_crop=1, test_num_segment=10, test_num_crop=3, args=None,
                 file_ext='mp4', task='classification'):
        super(VideoClsDatasetSegment, self).__init__(anno_path, data_path, mode, clip_len,
                 frame_sample_rate, crop_size, short_side_size,
                 new_height, new_width, keep_aspect_ratio,
                 num_segment, num_crop, test_num_segment, test_num_crop, args,
                 file_ext, task)
        self.num_segment = num_segment
        self.test_num_segment = test_num_segment
        self.test_num_crop = test_num_crop
    ...
    def load_video(self, sample, sample_rate_scale=1):
        ...
        vr = VideoReaderSegmentFrame(fname, file_ext=self.file_ext)
        ...
        for i in range(self.num_segment):
            if seg_len <= converted_len:
                ...
                index = np.concatenate(
                    (index, np.ones(self.clip_len - seg_len // self.frame_sample_rate) * seg_len))
                index = np.clip(index, 0, seg_len - 1).astype(np.int64)
            else:
                end_idx = np.random.randint(converted_len, seg_len)
                str_idx = end_idx - converted_len
...
class VideoReaderSegmentFrame():
    def __init__(self, video_dir, file_ext='jpg'):
        basename = os.path.basename(video_dir)
        segment_num = int(video_dir.split('_')[-1])
        videoname = '_'.join(basename.split('_')[:-1])
        
        self.video_dir = os.path.join(os.path.dirname(video_dir), videoname)
        self.frames = sorted(
            glob.glob(os.path.join(self.video_dir, f'*.{file_ext}')))
        self.frames = self.frames[segment_num*16:(segment_num+1)*16]
