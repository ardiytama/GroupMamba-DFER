#!/bin/bash
# Fine-tuning script for FERV39K dataset (local setup)
# Usage: bash scripts/FERV39K/ft_moe_ferv39k_local.sh [model] [device] [moe_type] [num_experts] [top_k] [moe_layers]
# Example: bash scripts/FERV39K/ft_moe_ferv39k_local.sh vitmoe_base_patch16_160 0 moe_adapters 8 2 6
...
for ckpt in "${ckpts[@]}";
do
    tag=${finetune_dataset}_${moe_type}_et${num_experts}_k${top_k}_moe_layer_id${moe_layers}-11
    ...
    # Data paths (relative to finetune/ directory)
    DATA_PATH="data/FERV39K/2_ClipsforFaceCrop"
    TRAIN_LABEL="data/FERV39K/Annotation/train.csv"
    TEST_LABEL="data/FERV39K/Annotation/test.csv"
    MODEL_PATH="${ckpt}"
