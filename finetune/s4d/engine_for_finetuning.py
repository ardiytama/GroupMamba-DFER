...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    start_steps=None, aux_loss_coeff=None, lr_schedule_values=None, wd_schedule_values=None,
    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if mixup_fn is not None and samples.ndim>4 :
        ...
        if loss_scaler is None:
            ...
            with torch.cuda.amp.autocast():
                loss, output = train_class_batch(
                    model, samples, targets, criterion)
                try:
                    aux_loss = model.get_aux_loss() * aux_loss_coeff
                    loss = loss + aux_loss
                except:
                    pass
...
@torch.no_grad()
def sfer_validation_one_epoch(data_loader, model, device):
    criterion = torch.nn.CrossEntropyLoss()
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        batch_size = videos.shape[0]
        metric_logger.update(sfer_loss=loss.item())
        metric_logger.meters['sfer_acc1'].update(acc1.item(), n=batch_size)
        metric_logger.meters['sfer_acc5'].update(acc5.item(), n=batch_size)
    # me: cal total metrics across the val set
    preds, labels = np.concatenate(outputs), np.concatenate(targets)
    preds = np.argmax(preds, axis=1)
    from sklearn.metrics import confusion_matrix, f1_score
    ...
    metric_logger.synchronize_between_processes()
    print('Class Acc:', class_acc)
    print('Val on SFER Dataset: * Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f} loss {losses.global_avg:.3f}'
          .format(top1=metric_logger.sfer_acc1, top5=metric_logger.sfer_acc5, losses=metric_logger.sfer_loss))

    print('Val on SFER Dataset: * WAR {war.global_avg:.4f} UAR {uar.global_avg:.4f} weighted_f1 {weighted_f1.global_avg:.4f} micro_f1 {micro_f1.global_avg:.4f} macro_f1 {macro_f1.global_avg:.4f}'
          .format(war=metric_logger.war, uar=metric_logger.uar, weighted_f1=metric_logger.weighted_f1, micro_f1=metric_logger.micro_f1, macro_f1=metric_logger.macro_f1))
        ...
        with torch.cuda.amp.autocast():
            ...
            if save_feature:
                output, saved_feature = model(
                    videos, save_feature=save_feature)
            else:
                output = model(videos)
